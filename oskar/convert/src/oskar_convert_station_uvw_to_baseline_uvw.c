...
#define CONVERT_STATION_TO_BASELINE(NAME, FP) static void NAME(\
        const int use_casa_phase_convention, const int num_stations,\
...
{\
    ...
    for (s1 = 0, b = 0; s1 < num_stations; ++s1) {\
        ...
        for (s2 = s1 + 1; s2 < num_stations; ++s2, ++b) {\
            ...
            uu[out] = factor * (u[i2] - u[i1]);\
            vv[out] = factor * (v[i2] - v[i1]);\
            ww[out] = factor * (w[i2] - w[i1]);\
        }\
    }\
}
...
)
{
    ...
    if (location == OSKAR_CPU)
    {
        ...
        {
            convert_station_uvw_to_baseline_uvw_double(
                    use_casa_phase_convention,
                    num_stations,
                    offset_in,
