...
)
{
    ...
    int i_healpix = 0, i_hdu = 0, num_cols = 0, num_hdu = 0;
    ...
    if (*status)
    {
        oskar_log_error(                                  /* LCOV_EXCL_LINE */
                0, "Failure in fits_get_num_hdus."        /* LCOV_EXCL_LINE */
        );
    ...
    for (i_hdu = 2; i_hdu <= num_hdu; ++i_hdu)
    {
        ...
        {
            oskar_log_error(                              /* LCOV_EXCL_LINE */
                    0, "Failure in fits_movabs_hdu."      /* LCOV_EXCL_LINE */
            );
            goto file_error;                              /* LCOV_EXCL_LINE */
        }
        if (hdutype != BINARY_TBL) continue;

        /* Look for XTENSION keyword. */
        fits_read_key(fptr, TSTRING, "XTENSION", card1, NULL, status);
        if (*status)
        {
            oskar_log_error(                              /* LCOV_EXCL_LINE */
                    0, "Failure in fits_read_key "        /* LCOV_EXCL_LINE */
                    "(XTENSION)."                         /* LCOV_EXCL_LINE */
            );
            goto file_error;                              /* LCOV_EXCL_LINE */
        }
    ...
    {
        oskar_log_error(                                  /* LCOV_EXCL_LINE */
                0, "Failed to get size of binary table."  /* LCOV_EXCL_LINE */
        );
        goto file_error;                                  /* LCOV_EXCL_LINE */
    }
    ...
    {
        oskar_log_error(                                  /* LCOV_EXCL_LINE */
                0, "Failure in fits_get_coltype."         /* LCOV_EXCL_LINE */
        );
        goto file_error;                                  /* LCOV_EXCL_LINE */
    }
    ...
    {
        oskar_log_error(                                  /* LCOV_EXCL_LINE */
                0, "Failure in fits_read_key (NSIDE)."    /* LCOV_EXCL_LINE */
        );
        goto file_error;                                  /* LCOV_EXCL_LINE */
    }
    ...
    {
        oskar_log_error(                                  /* LCOV_EXCL_LINE */
                0, "Failure in fits_read_key (ORDERING)." /* LCOV_EXCL_LINE */
        );
        goto file_error;                                  /* LCOV_EXCL_LINE */
    }
    *ordering = card1[0];
    ...
    {
        oskar_log_error(                                  /* LCOV_EXCL_LINE */
                0, "Failure in fits_read_key (COORDSYS)." /* LCOV_EXCL_LINE */
        );
        goto file_error;                                  /* LCOV_EXCL_LINE */
    }
    *coordsys = card1[0];
    ...
    {
        const size_t len = 1 + strlen(card1);
        if (!status1 && len > 0)
        {
            *brightness_units = (char*) realloc (*brightness_units, len);
            memcpy(*brightness_units, card1, len);
        }
    }
    ...
    fits_read_col(
            fptr, type_fits, col_index, 1, 1, num_pixels, 0,
            oskar_mem_void(data), 0, status
    );
    if (*status)
    {
        oskar_log_error(0, "Failure in fits_read_col.");  /* LCOV_EXCL_LINE */
