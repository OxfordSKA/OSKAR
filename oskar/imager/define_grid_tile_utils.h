...
#define GET_TILE_RANGES(U_MIN, U_MAX, V_MIN, V_MAX)\
        const int rel_u = grid_u - top_left_u;\
        const int rel_v = grid_v - top_left_v;\
        const float u1 = (float)(rel_u - w_support) * inv_tile_size_u;\
        const float u2 = (float)(rel_u + w_support + 1) * inv_tile_size_u;\
        const float v1 = (float)(rel_v - w_support) * inv_tile_size_v;\
        const float v2 = (float)(rel_v + w_support + 1) * inv_tile_size_v;\
 ...
 */
#define OSKAR_GRID_TILE_COUNT(NAME, FP) KERNEL(NAME) (\
...
{\
    ...
    if ((grid_u + w_support < grid_size) && (grid_u - w_support >= 0) &&\
        ...
        GET_TILE_RANGES(tile_u_min, tile_u_max, tile_v_min, tile_v_max)\
 ...
 */
#define OSKAR_GRID_TILE_BUCKET_SORT(NAME, FP, FP2) KERNEL(NAME) (\
        const int       num_w_planes,\
        GLOBAL_IN(int,  support),\
        const int       num_vis,\
        GLOBAL_IN(FP,   uu),\
