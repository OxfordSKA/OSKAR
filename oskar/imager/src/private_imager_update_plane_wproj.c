...
static void* run_subset(void* arg)
{
    ...
    oskar_mem_ensure(d->ww, num_points, status);
    ...
    oskar_mem_copy_contents(d->ww, ww, 0, start, num_points, status);
    ...
    const float inv_tile_size_v = 1.0f / (float) tile_size_v;
    {
        ...
        const oskar_Arg args[] = {
                ...
                {PTR_SZ, oskar_mem_buffer_const(d->ww)},
                {INT_SZ, &grid_size},
                {INT_SZ, &grid_centre},
    ...
    oskar_mem_ensure(d->sorted_ww, num_total, status);
    ...
    oskar_mem_ensure(d->sorted_tile, num_total, status);
    {
        ...
        const oskar_Arg args[] = {
                ...
                {PTR_SZ, oskar_mem_buffer(d->sorted_ww)},
    ...
    /* Update the grid. */
    {
        ...
        const oskar_Arg args[] = {
                ...
                {PTR_SZ, oskar_mem_buffer_const(d->sorted_ww)},
