...
)
{
    ...
    if (naxis == 0)
    {
        ...
        data = oskar_mem_read_healpix_fits(
                filename, 0, &nside, &ordering, &coordsys,
                &reported_map_units, status
        );
        if (data) pixel_area_sr = (4.0 * M_PI) / oskar_mem_length(data);
        ...
        if (!*status && ordering != 'R')
        {
            *status = OSKAR_ERR_FILE_IO;                  /* LCOV_EXCL_LINE */
            oskar_log_error(                              /* LCOV_EXCL_LINE */
                    0, "HEALPix data not in RING format." /* LCOV_EXCL_LINE */
            );
        }
    }
    else
    {
        ...
        data = oskar_mem_read_fits_image_plane(
                filename, 0, 0, 0, image_size, image_crval_deg, image_crpix,
    ...
    oskar_convert_brightness_to_jy(
            data, beam_area_pixels, pixel_area_sr, image_freq_hz,
            min_peak_fraction, min_abs_val, reported_map_units,
    ...
    if (naxis == 0)
    {
        t = oskar_sky_from_healpix_ring(
                precision, data, image_freq_hz, spectral_index,
                nside, (coordsys == 'G'), status
        );
    }
    else
    {
        t = oskar_sky_from_image(
                precision, data, image_size, image_crval_deg, image_crpix,
