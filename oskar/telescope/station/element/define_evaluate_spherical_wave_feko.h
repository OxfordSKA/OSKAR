 ...
 *
 * Modified for FEKO conventions by Maciej Serylak.
 *
 * See "Spherical near-field antenna measurements", J. E. Hansen, 1988
 */

#define OSKAR_SPH_WAVE2(FP2, M, A_TE, A_TM, C_THETA, C_PHI) {\
    ...
    OSKAR_MUL_SUB_COMPLEX(C_THETA, qq_tm, A_TM)\
    OSKAR_MUL_ADD_COMPLEX(C_PHI, dd_tm, A_TM)\
    OSKAR_MUL_ADD_COMPLEX(C_PHI, qq_te, A_TE)\
    }\

#define OSKAR_EVALUATE_SPHERICAL_WAVE_SUM_FEKO(NAME, FP, FP2, FP4c)\
KERNEL(NAME) (\
        ...
        for (int l = 1; l <= l_max; ++l) {\
            ...
            for (int abs_m = l; abs_m >=0; --abs_m) {\
                ...
                if (abs_m == 0) {\
                    ...
                    OSKAR_SPH_WAVE2(FP2, 0, alpha_.a, alpha_.b, Xt, Xp)\
                    OSKAR_SPH_WAVE2(FP2, 0, alpha_.c, alpha_.d, Yt, Yp)\
                }\
                else {\
                    ...
                    sin_p *= nf; cos_p *= nf;\
                    OSKAR_SPH_WAVE2(FP2, -abs_m, alpha_m.a, alpha_m.b, Xt, Xp)\
                    bh_factor = pow((FP)-1, (FP)abs_m);\
                    sin_p = -sin_p;\
                    OSKAR_SPH_WAVE2(FP2,  abs_m, alpha_p.a, alpha_p.b, Xt, Xp)\
                    bh_factor = (FP)1;\
                    p = -abs_m * phi_y_;\
                    SINCOS(p, sin_p, cos_p);\
                    sin_p *= nf; cos_p *= nf;\
                    OSKAR_SPH_WAVE2(FP2, -abs_m, alpha_m.c, alpha_m.d, Yt, Yp)\
                    sin_p = -sin_p;\
                    bh_factor = pow((FP)-1, (FP)abs_m);\
                    OSKAR_SPH_WAVE2(FP2,  abs_m, alpha_p.c, alpha_p.d, Yt, Yp)\
                }\
            }\
        }\
    }\
