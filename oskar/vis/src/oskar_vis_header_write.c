...
oskar_Binary* oskar_vis_header_write(
...
)
{
    ...
    oskar_binary_write_int(h, grp,
            OSKAR_VIS_HEADER_TAG_CASA_PHASE_CONVENTION, 0,
            hdr->casa_phase_convention, status
    );
    oskar_binary_write_int(h, grp,
    ...
    for (i = 0; i < hdr->num_stations; ++i)
    {
        oskar_binary_write_mem(h, hdr->station_name[i], grp,
                OSKAR_VIS_HEADER_TAG_STATION_NAME, i, 0, status
        );
    }

    /* Write station diameters. */
    oskar_binary_write_mem(h, hdr->station_diameter_m, grp,
            OSKAR_VIS_HEADER_TAG_STATION_DIAMETER, 0, 0, status
    );

    /* Write station element feed angles. */
    for (i = 0; i < hdr->num_stations; ++i)
    {
        oskar_binary_write_mem(h, hdr->element_euler_angle_rad[0][0][i], grp,
                OSKAR_VIS_HEADER_TAG_ELEMENT_FEED_ANGLE_X_A, i, 0, status
        );
        oskar_binary_write_mem(h, hdr->element_euler_angle_rad[1][0][i], grp,
                OSKAR_VIS_HEADER_TAG_ELEMENT_FEED_ANGLE_Y_A, i, 0, status
        );
        oskar_binary_write_mem(h, hdr->element_euler_angle_rad[0][1][i], grp,
                OSKAR_VIS_HEADER_TAG_ELEMENT_FEED_ANGLE_X_B, i, 0, status
        );
        oskar_binary_write_mem(h, hdr->element_euler_angle_rad[1][1][i], grp,
                OSKAR_VIS_HEADER_TAG_ELEMENT_FEED_ANGLE_Y_B, i, 0, status
        );
        oskar_binary_write_mem(h, hdr->element_euler_angle_rad[0][2][i], grp,
                OSKAR_VIS_HEADER_TAG_ELEMENT_FEED_ANGLE_X_C, i, 0, status
        );
        oskar_binary_write_mem(h, hdr->element_euler_angle_rad[1][2][i], grp,
                OSKAR_VIS_HEADER_TAG_ELEMENT_FEED_ANGLE_Y_C, i, 0, status
        );
    }
