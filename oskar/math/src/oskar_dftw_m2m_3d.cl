...
kernel void dftw_m2m_3d_REAL(const int num_in,
    ...
    for (int j = 0; j < num_in; j += max_in_chunk) {
        ...
        if (i_out < num_out) {
            for (int i = 0; i < chunk_size; ++i) {
                REAL re, im, t;
                REAL2 w = c_w[i];
                t = xo * c_xy[i].x + yo * c_xy[i].y + zo * c_z[i];
                im = sincos(t, &re);
                t = re;
                re *= w.x;
                re -= w.y * im;
...
kernel void dftw_m2m_3d_cpu_REAL(const int num_in,
        const REAL wavenumber,
        global const REAL* restrict x_in,
        global const REAL* restrict y_in,
        global const REAL* restrict z_in,
        ...
        global REAL8* restrict output)
{
    ...
    for (int i = 0; i < num_in; ++i) {
        ...
        t = xo * x_in[i] + yo * y_in[i] + zo * z_in[i];
        im = sincos(t, &re);
        t = re;
        re *= w.x;
        re -= w.y * im;
