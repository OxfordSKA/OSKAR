...
void fpsurf_f(int iopt, int m, float *x, float *y, const float *z,
        ...
        float *wrk, int lwrk, int *ier)
{
    ...
    float acc = 0.0, arg, cos_, dmax, fac1, fac2, fpmax, fpms = 0.0, f1, f2, f3;
    float eps, hxi, p, pinv, piv, p1, p2, p3, sigma, sin_, sq, store, wi;
    float rn, hx[6], hy[6], x0, x1, y0, y1, zi, con1, con4, con9;
    ...
    for (iter = 1; iter <= m; ++iter)
    {
        ...
        fporde_f(&x[1], &y[1], m, kx, ky, &tx[1], nx, &ty[1], ny,
                &nummer[1], &index[1], nreg);
        ...
        *fp = 0.0;
            ...
            while (in != 0)
            {
                ...
                for (i = 1; i <= iband; ++i)
                {
                    ...
                    for (j = i3; j <= iband; ++j)
                    {
                        ...
                        fprota_f(cos_, sin_, &h[j], &a[irot + i2 * nc]);
                    }
                }
        ...
        if (i != -1)
        {
            ...
            fpback_f(&a[a_offset], &f[1], ncof, iband, &c[1], nc);
            rank = ncof;
            for (i = 1; i <= ncof; ++i)
            {
                q[i + nc] = a[i + nc] / dmax;
        ...
        else
        {
            ...
            fprank_f(&q[q_offset], &ff[1], ncof, iband, nc, sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]);
            for (i = 1; i <= ncof; ++i)
            {
                q[i + nc] /= dmax;
    ...
    if (nk1x != kx1)
    {
        ...
        fpdisc_f(&tx[1], nx, kx2, &bx[bx_offset], nmax);
    }
    ky2 = ky1 + 1;
    ...
    if (nk1y != ky1)
    {
        ...
        fpdisc_f(&ty[1], ny, ky2, &by[by_offset], nmax);
    }
    /*  initial value for p. */
    p1 = 0.0;
    f1 = *fp0 - s;
    ...
    for (iter = 1; iter <= maxit; ++iter)
    {
                    ...
                    for (irot = jrot; irot <= ncof; ++irot)
                    {
                        ...
                        else
                        {
                            ...
                            for (l = 1; l <= i2; ++l)
                            {
                                ...
                                fprota_f(cos_, sin_, &h[l1], &q[irot + l1 * nc]);
                            }
                        }
                        for (l = 1; l <= i2; ++l)
                        {
        ...
        if (i != -1)
        {
            ...
            fpback_f(&q[q_offset], &ff[1], ncof, iband4, &c[1], nc);
            rank = ncof;
        }
        else
        {
            ...
            /* fprank_(&q[q_offset], &ff[1], &ncof, &iband4, &nc, &sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]); */
            fprank_f(&q[q_offset], &ff[1], ncof, iband4, nc, sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]);
        }
        for (i = 1; i <= ncof; ++i)
        {
...
void fpsurf_d(int iopt, int m, double *x, double *y, const double *z,
        ...
        double *wrk, int lwrk, int *ier)
{
    ...
    for (iter = 1; iter <= m; ++iter)
    {
        ...
        /* fporde_(&x[1], &y[1], &m, &kx, &ky, &tx[1], &nx, &ty[1], &ny,
                &nummer[1], &index[1], &nreg); */
        fporde_d(&x[1], &y[1], m, kx, ky, &tx[1], nx, &ty[1], ny,
                &nummer[1], &index[1], nreg);
        /*  find ncof, the number of b-spline coefficients. */
        nk1x = nx - kx1;
        nk1y = ny - ky1;
        ...
        *fp = 0.0;
        ...
        for (num = 1; num <= nreg; ++num)
        {
            ...
            while (in != 0)
            {
                 ...
                 *  b-splines at x(in). */
                /* fpbspl_(&tx[1], &nx, &kx, &x[in], &l1, hx); */
                fpbspl_d(&tx[1], kx, x[in], l1, hx);
                /*  evaluate for the y-direction, the (ky+1) non-zero
                 *  b-splines at y(in). */
                /* fpbspl_(&ty[1], &ny, &ky, &y[in], &l2, hy); */
                fpbspl_d(&ty[1], ky, y[in], l2, hy);
                /*  store the value of these b-splines in spx and spy
                 *  respectively. */
                for (i = 1; i <= kx1; ++i)
                {
                ...
                for (i = 1; i <= iband; ++i)
                {
                     ...
                     *  transformation. */
                    /* fpgivs_(&piv, &a[irot + nc], &cos_, &sin_); */
                    fpgivs_d(piv, &a[irot + nc], &cos_, &sin_);
                    /*  apply that transformation to the right hand side. */
                    /* fprota_(&cos_, &sin_, &zi, &f[irot]); */
                    fprota_d(cos_, sin_, &zi, &f[irot]);
                    if (i == iband) break;
                    /*  apply that transformation to the left hand side. */
                    i2 = 1;
                    i3 = i + 1;
                    for (j = i3; j <= iband; ++j)
                    {
                        ++i2;
                        /* fprota_(&cos_, &sin_, &h[j], &a[irot + i2 * nc]); */
                        fprota_d(cos_, sin_, &h[j], &a[irot + i2 * nc]);
                    }
                }
                /*  add the contribution of the row to the sum of squares
                 *  of residual right hand sides. */
        ...
        if (i != -1)
        {
            /*  backward substitution in case of full rank. */
            /* fpback_(&a[a_offset], &f[1], &ncof, &iband, &c[1], &nc); */
            fpback_d(&a[a_offset], &f[1], ncof, iband, &c[1], nc);
            rank = ncof;
            for (i = 1; i <= ncof; ++i)
            {
                q[i + nc] = a[i + nc] / dmax;
        ...
        else
        {
            ...
            /* fprank_(&q[q_offset], &ff[1], &ncof, &iband, &nc, &sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]); */
            fprank_d(&q[q_offset], &ff[1], ncof, iband, nc, sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]);
            for (i = 1; i <= ncof; ++i)
            {
                q[i + nc] /= dmax;
    ...
    if (nk1x != kx1)
    {
         ...
         *  the b-splines at the knots tx(l),l=kx+2,...,nx-kx-1. */
        /* fpdisc_(&tx[1], &nx, &kx2, &bx[bx_offset], &nmax); */
        fpdisc_d(&tx[1], nx, kx2, &bx[bx_offset], nmax);
    }
    ky2 = ky1 + 1;
    /*  test whether there are interior knots in the y-direction. */
    if (nk1y != ky1)
    {
         ...
         *  the b-splines at the knots ty(l),l=ky+2,...,ny-ky-1. */
        /* fpdisc_(&ty[1], &ny, &ky2, &by[by_offset], &nmax); */
        fpdisc_d(&ty[1], ny, ky2, &by[by_offset], nmax);
    }
    /*  initial value for p. */
    p1 = 0.0;
    f1 = *fp0 - s;
    ...
    for (iter = 1; iter <= maxit; ++iter)
    {
                ...
                for (j = 1; j <= nk1x; ++j)
                {
                    ...
                    for (irot = jrot; irot <= ncof; ++irot)
                    {
                        ...
                        else
                        {
                             ...
                             *  transformation. */
                            /* fpgivs_(&piv, &q[irot + nc], &cos_, &sin_); */
                            fpgivs_d(piv, &q[irot + nc], &cos_, &sin_);
                            /*  apply that givens transformation to the right
                             *  hand side. */
                            /* fprota_(&cos_, &sin_, &zi, &ff[irot]); */
                            fprota_d(cos_, sin_, &zi, &ff[irot]);
                            if (i2 == 0) break;
                            /*  apply that givens transformation to the left
                             *  hand side. */
                            for (l = 1; l <= i2; ++l)
                            {
                                l1 = l + 1;
                                /* fprota_(&cos_, &sin_, &h[l1], &q[irot + l1 * nc]); */
                                fprota_d(cos_, sin_, &h[l1], &q[irot + l1 * nc]);
                            }
                        }
                        for (l = 1; l <= i2; ++l)
                        {
        ...
        if (i != -1)
        {
            /*  backward substitution in case of full rank. */
            /* fpback_(&q[q_offset], &ff[1], &ncof, &iband4, &c[1], &nc); */
            fpback_d(&q[q_offset], &ff[1], ncof, iband4, &c[1], nc);
            rank = ncof;
        }
        else
        {
            ...
            /* fprank_(&q[q_offset], &ff[1], &ncof, &iband4, &nc, &sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]); */
            fprank_d(&q[q_offset], &ff[1], ncof, iband4, nc, sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]);
        }
        for (i = 1; i <= ncof; ++i)
        {
