...
void bispev_f(const float *tx, int nx, const float *ty, int ny,
    ...
    int my, float *z, float *wrk, int lwrk, int *iwrk, int kwrk, int *ier)
{
    ...
    *ier = 0;
    iw = mx * (kx + 1);
    fpbisp_f(tx, nx, ty, ny, c, kx, ky, x, mx, y, my,
        z, wrk, &wrk[iw], iwrk, &iwrk[mx]);
}

void bispev_d(const double *tx, int nx, const double *ty, int ny,
    ...
    int my, double *z, double *wrk, int lwrk, int *iwrk, int kwrk, int *ier)
{
    ...
    *ier = 0;
    iw = mx * (kx + 1);
    fpbisp_d(tx, nx, ty, ny, c, kx, ky, x, mx, y, my,
        z, wrk, &wrk[iw], iwrk, &iwrk[mx]);
}
