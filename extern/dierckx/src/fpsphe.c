    ...
    *sup = 0.0;
    ...
    for (i = 1; i <= m; ++i)
    {
        ...
        if (fn != 0.0)
        {
            ...
            /* fprota_(&co, &si, &f1, &aa); */
            /* fprota_(&co, &si, &ri, &cn); */
            fpgivs_f(fn, &d1, &co, &si);
            fprota_f(co, si, &f1, &aa);
            fprota_f(co, si, &ri, &cn);
        }
        if (f1 != 0.0)
        {
            /* fpgivs_(&f1, &d2, &co, &si); */
            /* fprota_(&co, &si, &ri, &c1); */
            fpgivs_f(f1, &d2, &co, &si);
            fprota_f(co, si, &ri, &c1);
        }
        *sup += ri * ri;
    }
    if (d2 != 0.0) c1 /= d2;
...
L70:
    ...
    for (iter = 1; iter <= m; ++iter)
    {
        ...
        fporde_f(&theta[1], &phi[1], m, 3, 3, &tt[1], *nt, &tp[1], *np,
                &nummer[1], &index[1], nreg);
        ...
        for (i = 1; i <= npp; ++i)
        {
            ...
            for (j = 1; j <= npp; ++j)
            {
                ...
                fpgivs_f(piv, &a[j + a_dim1], &co, &si);
                fprota_f(co, si, &facc, &coco[j]);
                fprota_f(co, si, &facs, &cosi[j]);
                if (j == npp) break;
                j1 = j + 1;
                i2 = 1;
                for (l = j1; l <= npp; ++l)
                {
                    ...
                    fprota_f(co, si, &row[l], &a[j + i2 * a_dim1]);
                }
            }
        }
        /* fpback_(&a[a_offset], &coco[1], &npp, &npp, &coco[1], &ncc); */
        /* fpback_(&a[a_offset], &cosi[1], &npp, &npp, &cosi[1], &ncc); */
        fpback_f(&a[a_offset], &coco[1], npp, npp, &coco[1], ncc);
        fpback_f(&a[a_offset], &cosi[1], npp, npp, &cosi[1], ncc);
        ...
        *fp = 0.0;
            ...
            while (in != 0)
            {
                ...
                for (i = 1; i <= iband; ++i)
                {
                    ...
                    for (j = i3; j <= iband; ++j)
                    {
                        ...
                        fprota_f(co, si, &h[j], &a[irot + i2 * a_dim1]);
                    }
                }
        ...
        if (i != -1)
        {
            ...
            fpback_f(&a[a_offset], &f[1], ncof, iband, &c[1], ncc);
            rank = ncof;
            for (i = 1; i <= ncof; ++i)
            {
        ...
        else
        {
            ...
            fprank_f(&q[q_offset], &ff[1], ncof, iband, ncc, sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]);
            for (i = 1; i <= ncof; ++i)
            {
                q[i + q_dim1] /= dmax;
        ...
        fprpsp_f(*nt, *np, &coco[1], &cosi[1], &c[1], &ff[1], ncoff);
    ...
    for (iter = 1; iter <= maxit; ++iter)
    {
                ...
                for (irot = jrot; irot <= ncof; ++irot)
                {
                    ...
                    else
                    {
                        ...
                        for (l = 1; l <= i2; ++l)
                        {
                            ...
                            fprota_f(co, si, &h[l1], &q[irot + l1 * q_dim1]);
                        }
                    }
                    for (l = 1; l <= i2; ++l)
                    {
        ...
        if (i != -1)
        {
            ...
            fpback_f(&q[q_offset], &ff[1], ncof, iband4, &c[1], ncc);
            rank = ncof;
        }
        else
        {
            ...
            /* fprank_(&q[q_offset], &ff[1], &ncof, &iband4, &ncc, &sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]); */
            fprank_f(&q[q_offset], &ff[1], ncof, iband4, ncc, sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]);
        }
        for (i = 1; i <= ncof; ++i)
        {
    ...
    *sup = 0.0;
    ...
    for (i = 1; i <= m; ++i)
    {
        ...
        if (fn != 0.0)
        {
            ...
            /* fprota_(&co, &si, &f1, &aa); */
            /* fprota_(&co, &si, &ri, &cn); */
            fpgivs_d(fn, &d1, &co, &si);
            fprota_d(co, si, &f1, &aa);
            fprota_d(co, si, &ri, &cn);
        }
        if (f1 != 0.0)
        {
            /* fpgivs_(&f1, &d2, &co, &si); */
            /* fprota_(&co, &si, &ri, &c1); */
            fpgivs_d(f1, &d2, &co, &si);
            fprota_d(co, si, &ri, &c1);
        }
        *sup += ri * ri;
    }
    if (d2 != 0.0) c1 /= d2;
...
L70:
    ...
    for (iter = 1; iter <= m; ++iter)
    {
        ...
        /* fporde_(&theta[1], &phi[1], &m, &c3, &c3, &tt[1], nt, &tp[1], np,
                &nummer[1], &index[1], &nreg); */
        fporde_d(&theta[1], &phi[1], m, 3, 3, &tt[1], *nt, &tp[1], *np,
                &nummer[1], &index[1], nreg);
        /*  find the b-spline coefficients coco and cosi of the cubic spline
         *  approximations sc(phi) and ss(phi) for cos(phi) and sin(phi). */
        for (i = 1; i <= npp; ++i)
        ...
        for (i = 1; i <= npp; ++i)
        {
            ...
            for (j = 1; j <= npp; ++j)
            {
                ...
                /* fprota_(&co, &si, &facc, &coco[j]); */
                /* fprota_(&co, &si, &facs, &cosi[j]); */
                fpgivs_d(piv, &a[j + a_dim1], &co, &si);
                fprota_d(co, si, &facc, &coco[j]);
                fprota_d(co, si, &facs, &cosi[j]);
                if (j == npp) break;
                j1 = j + 1;
                i2 = 1;
                for (l = j1; l <= npp; ++l)
                {
                    ++i2;
                    /* fprota_(&co, &si, &row[l], &a[j + i2 * a_dim1]); */
                    fprota_d(co, si, &row[l], &a[j + i2 * a_dim1]);
                }
            }
        }
        /* fpback_(&a[a_offset], &coco[1], &npp, &npp, &coco[1], &ncc); */
        /* fpback_(&a[a_offset], &cosi[1], &npp, &npp, &cosi[1], &ncc); */
        fpback_d(&a[a_offset], &coco[1], npp, npp, &coco[1], ncc);
        fpback_d(&a[a_offset], &cosi[1], npp, npp, &cosi[1], ncc);
        /*  find ncof, the dimension of the spherical spline and ncoff, the
         *  number of coefficients in the standard b-spline representation. */
        nt4 = *nt - 4;
        np4 = *np - 4;
        ...
        *fp = 0.0;
        ...
        for (num = 1; num <= nreg; ++num)
        {
            ...
            while (in != 0)
            {
                 ...
                 *  at theta(in) */
                /* fpbspl_(&tt[1], nt, &c3, &theta[in], &l1, ht); */
                fpbspl_d(&tt[1], 3, theta[in], l1, ht);
                /*  evaluate for the phi-direction, the 4 non-zero b-splines
                 *  at phi(in) */
                /* fpbspl_(&tp[1], np, &c3, &phi[in], &l2, hp); */
                fpbspl_d(&tp[1], 3, phi[in], l2, hp);
                /*  store the value of these b-splines in spt and spp resp. */
                for (i = 1; i <= 4; ++i)
                {
                    spp[in + i * m] = hp[i - 1];
                ...
                for (i = 1; i <= iband; ++i)
                {
                    ...
                    /*  calculate the parameters of the givens transformation. */
                    /* fpgivs_(&piv, &a[irot + a_dim1], &co, &si); */
                    fpgivs_d(piv, &a[irot + a_dim1], &co, &si);
                    /*  apply that transformation to the right hand side. */
                    /* fprota_(&co, &si, &ri, &f[irot]); */
                    fprota_d(co, si, &ri, &f[irot]);
                    if (i == iband) break;
                    /*  apply that transformation to the left hand side. */
                    i2 = 1;
                    i3 = i + 1;
                    for (j = i3; j <= iband; ++j)
                    {
                        ++i2;
                        /* fprota_(&co, &si, &h[j], &a[irot + i2 * a_dim1]); */
                        fprota_d(co, si, &h[j], &a[irot + i2 * a_dim1]);
                    }
                }
                /*  add the contribution of the row to the sum of squares of
                 *  residual right hand sides. */
        ...
        if (i != -1)
        {
            /*  backward substitution in case of full rank. */
            /* fpback_(&a[a_offset], &f[1], &ncof, &iband, &c[1], &ncc); */
            fpback_d(&a[a_offset], &f[1], ncof, iband, &c[1], ncc);
            rank = ncof;
            for (i = 1; i <= ncof; ++i)
            {
                q[i + q_dim1] = a[i + a_dim1] / dmax;
        ...
        else
        {
            ...
            /* fprank_(&q[q_offset], &ff[1], &ncof, &iband, &ncc, &sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]); */
            fprank_d(&q[q_offset], &ff[1], ncof, iband, ncc, sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]);
            for (i = 1; i <= ncof; ++i)
            {
                q[i + q_dim1] /= dmax;
         ...
         *  the spherical spline. */
        /* fprpsp_(nt, np, &coco[1], &cosi[1], &c[1], &ff[1], &ncoff); */
        fprpsp_d(*nt, *np, &coco[1], &cosi[1], &c[1], &ff[1], ncoff);
        /*  test whether the least-squares spline is an acceptable solution. */
        if (iopt < 0) {
            if (*fp <= 0.0) {
                goto L970;
     ...
     *  the b-splines at the knots tt(l),l=5,...,nt-4. */
    /* fpdisc_(&tt[1], nt, &c5, &bt[bt_offset], &ntest); */
    fpdisc_d(&tt[1], *nt, 5, &bt[bt_offset], ntest);
    /*  evaluate the discontinuity jumps of the 3rd order derivative of
     *  the b-splines at the knots tp(l),l=5,...,np-4. */
    /* fpdisc_(&tp[1], np, &c5, &bp[bp_offset], &npest); */
    fpdisc_d(&tp[1], *np, 5, &bp[bp_offset], npest);
    /*  initial value for p. */
    p1 = 0.0;
    f1 = *sup - s;
    p3 = -1.0;
    ...
    for (iter = 1; iter <= maxit; ++iter)
    {
            ...
            for (j = 1; j <= nt6; ++j)
            {
                ...
                for (irot = jrot; irot <= ncof; ++irot)
                {
                    ...
                    else
                    {
                         ...
                         *  transformation. */
                        /* fpgivs_(&piv, &q[irot + q_dim1], &co, &si); */
                        fpgivs_d(piv, &q[irot + q_dim1], &co, &si);
                        /*  apply givens transformation to right hand side. */
                        /* fprota_(&co, &si, &ri, &ff[irot]); */
                        fprota_d(co, si, &ri, &ff[irot]);
                        if (i2 == 0) break;
                        /*  apply givens transformation to left hand side. */
                        for (l = 1; l <= i2; ++l)
                        {
                            l1 = l + 1;
                            /* fprota_(&co, &si, &h[l1], &q[irot + l1 * q_dim1]); */
                            fprota_d(co, si, &h[l1], &q[irot + l1 * q_dim1]);
                        }
                    }
                    for (l = 1; l <= i2; ++l)
                    {
        ...
        if (i != -1)
        {
            /*  backward substitution in case of full rank. */
            /* fpback_(&q[q_offset], &ff[1], &ncof, &iband4, &c[1], &ncc); */
            fpback_d(&q[q_offset], &ff[1], ncof, iband4, &c[1], ncc);
            rank = ncof;
        }
        else
        {
            ...
            /* fprank_(&q[q_offset], &ff[1], &ncof, &iband4, &ncc, &sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]); */
            fprank_d(&q[q_offset], &ff[1], ncof, iband4, ncc, sigma, &c[1],
                    &sq, &rank, &wrk[la], &wrk[lf], &wrk[lh]);
        }
        for (i = 1; i <= ncof; ++i)
        {
