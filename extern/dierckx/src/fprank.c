    ...
    for (i = 1; i <= n; ++i)
    {
        ...
        for (ii = i1; ii <= n; ++ii)
        {
            ...
            else
            {
                ...
                fpgivs(piv, &a[ii + a_dim1], &cos_, &sin_);
                /* fprota_(&cos_, &sin_, &yi, &f[ii]); */
                fprota(cos_, sin_, &yi, &f[ii]);
                if (i2 == 0) break;
                for (j = 1; j <= i2; ++j)
                {
                    j1 = j + 1;
                    /* fprota_(&cos_, &sin_, &h[j1], &a[ii + j1 * a_dim1]); */
                    fprota(cos_, sin_, &h[j1], &a[ii + j1 * a_dim1]);
                    h[j] = h[j1];
                }
            }
            h[i2 + 1] = 0.0;
    ...
    for (i = 1; i <= n; ++i)
    {
        ...
        for (i1 = 1; i1 <= ii; ++i1)
        {
            ...
            if (piv != 0.0)
            {
                ...
                fpgivs(piv, &aa[jj + aa_dim1], &cos_, &sin_);
                if (j1 == 0) break;
                kk = jj;
                for (j2 = 1; j2 <= j1; ++j2)
                {
                    ...
                    fprota(cos_, sin_, &h[j3], &aa[kk + j3 * aa_dim1]);
                    h[j2] = h[j3];
                }
            }
            else
