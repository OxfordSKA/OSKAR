...
void fprank_f(float *a, float *f, int n, int m, int na,
    ...
    for (i = 1; i <= n; ++i)
    {
        ...
        for (ii = i1; ii <= n; ++ii)
        {
            ...
            else
            {
                ...
                fpgivs_f(piv, &a[ii + a_dim1], &cos_, &sin_);
                /* fprota_(&cos_, &sin_, &yi, &f[ii]); */
                fprota_f(cos_, sin_, &yi, &f[ii]);
                if (i2 == 0) break;
                for (j = 1; j <= i2; ++j)
                {
                    j1 = j + 1;
                    /* fprota_(&cos_, &sin_, &h[j1], &a[ii + j1 * a_dim1]); */
                    fprota_f(cos_, sin_, &h[j1], &a[ii + j1 * a_dim1]);
                    h[j] = h[j1];
                }
            }
            h[i2 + 1] = 0.0;
    ...
    for (i = 1; i <= n; ++i)
    {
        ...
        for (i1 = 1; i1 <= ii; ++i1)
        {
            ...
            if (piv != 0.0)
            {
                ...
                fpgivs_f(piv, &aa[jj + aa_dim1], &cos_, &sin_);
                if (j1 == 0) break;
                kk = jj;
                for (j2 = 1; j2 <= j1; ++j2)
                {
                    ...
                    fprota_f(cos_, sin_, &h[j3], &aa[kk + j3 * aa_dim1]);
                    h[j2] = h[j3];
                }
            }
            else
...
void fprank_d(double *a, double *f, int n, int m, int na,
    ...
    *sq = 0.0;
    for (i = 1; i <= n; ++i)
    {
        ...
        for (ii = i1; ii <= n; ++ii)
        {
            ...
            {
                /* fpgivs_(&piv, &a[ii + a_dim1], &cos_, &sin_); */
                fpgivs_d(piv, &a[ii + a_dim1], &cos_, &sin_);
                /* fprota_(&cos_, &sin_, &yi, &f[ii]); */
                fprota_d(cos_, sin_, &yi, &f[ii]);
                if (i2 == 0) break;
                for (j = 1; j <= i2; ++j)
                {
                    j1 = j + 1;
                    /* fprota_(&cos_, &sin_, &h[j1], &a[ii + j1 * a_dim1]); */
                    fprota_d(cos_, sin_, &h[j1], &a[ii + j1 * a_dim1]);
                    h[j] = h[j1];
                }
            }
            h[i2 + 1] = 0.0;
    ...
    *rank = n - nl;
    ...
    for (i = 1; i <= n; ++i)
    {
        ...
        for (i1 = 1; i1 <= ii; ++i1)
        {
            ...
            {
                /* fpgivs_(&piv, &aa[jj + aa_dim1], &cos_, &sin_); */
                fpgivs_d(piv, &aa[jj + aa_dim1], &cos_, &sin_);
                if (j1 == 0) break;
                kk = jj;
                for (j2 = 1; j2 <= j1; ++j2)
                {
                    ...
                    --kk;
                    /* fprota_(&cos_, &sin_, &h[j3], &aa[kk + j3 * aa_dim1]); */
                    fprota_d(cos_, sin_, &h[j3], &aa[kk + j3 * aa_dim1]);
                    h[j2] = h[j3];
                }
            }
            else
